...
void camera_up(vec3 out) {
    ...
    if (mat4x4_equals(_, _lookat)) {
        vec3_dup(out, (vec3) {0.0f, 1.0f, 0.0f});
    }
    else {
        mat4x4 lookat_t; mat4x4_transpose(lookat_t, _lookat);
...
void camera_get_projection(mat4x4 proj) {
    mat4x4 perspective;
    mat4x4_look_at(_lookat, _position, _target, (vec3) {0, 1, 0});
    mat4x4_perspective(perspective, RADIANS(45.0f), RENDER_WINDOW_ASPECT, 0.01f, 100.0f);
    mat4x4_mul(proj, perspective, _lookat);
}
...
void camera_rotate(float dx, float dy) {
    ...
    mat4x4 yaw;
    mat4x4 _; mat4x4_identity(_);
    mat4x4_rotate(yaw, _, 0, 1, 0, -dx_rad); // rotate using world up
    ...
    vec3 np;
    vec3_add(np, _target, pos_yaw);
    mat4x4_look_at(_lookat, np, _target, (vec3) {0, 1, 0});
    vec3 up; camera_up(up);
    vec3 forward; camera_forward(forward);
